...
export function BrandCard({ brand, removing, onRemove }: BrandCardProps) {
  ...
  return (
    <>
      <div className="flex items-center gap-7 p-3 rounded-lg border border-border bg-card justify-between max-w-lg">
        <div className="flex flex-col gap-6">
          ...
          {!brand.logo_filename && (
            <div className="w-24 h-10 rounded border border-dashed border-border flex items-center justify-center text-xs text-muted-foreground">
              No logo
            </div>
          )}
        </div>
